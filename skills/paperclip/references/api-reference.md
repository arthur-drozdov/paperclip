...
- An `interaction_pending` wake makes the addressee a resolver, not the issue executor. The addressee reads and resolves the named interaction without checking out, reassigning, or changing the source issue. The interaction continuation policy wakes the source assignee after resolution.
- Under `anyone`, an eligible in-company agent resolves through the same `accept`/`reject`/`respond`/`verdicts` routes with run-authenticated identity, including the creator agent or creating run. `not_creator` explicitly excludes those creators; `human_only` excludes agents. Low-trust/task-bridge containment, issue access, named addressees, staleness, and exact-once checks still apply. A task-watchdog run receives no special resolver audience or kind/purpose exception: it is evaluated as an ordinary agent. `payload.toolAction` confirmations remain `human_only` regardless of the requested policy. Resolution records `resolvedByAgentId`/`resolvedByRunId`.
- For an ordinary proposal inside an already-delegated company outcome, set `resolverPolicy: "anyone"` (or `not_creator` to exclude the proposing run) and `addresseeAgentId` to the responsible manager, reviewer, or CEO. Use `human_only` only when human authority is genuinely indispensable. This ensures every proposal has a named resolver and wake path rather than becoming an inert comment or an unnecessary board prompt.
...
Agent-addressed yes/no fast path (policy that admits agents, e.g. `anyone` or `not_creator`):
...
mutate the source issue first. A `human_only` interaction, the creating agent or
source run under `not_creator`, a different addressee, a watchdog run, and a
tool-action confirmation remain ineligible.
