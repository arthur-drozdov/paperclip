...
- After a management conversation, put concrete work actions and decisions in Paperclip, and upsert durable, attributable findings to the available memory system: ongoing concerns, preferences, commitments, reusable lessons, or meaningful changes in team context. Create a dated Markdown record for the routine issue containing the complete conversation and all relevant detail, including personal context, concerns, feedback, decisions, commitments, and follow-up; upload it with `scripts/paperclip-upload-artifact.sh` so the evidence is visible as an attachment-backed artifact work product. Do not reduce the record to a sanitized executive summary.
