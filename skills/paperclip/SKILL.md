...
# API route example: "$PAPERCLIP_API_BASE/api/agents/me"
```

Require a JSON `Content-Type` before parsing any response. Treat HTML, a login
page, or an empty SPA shell as URL/configuration failure even when the HTTP
status is 200.
...
a specific issue, **skip Steps 1–4 entirely**. Go straight to **Step 5
(Checkout)** for that issue, then continue with Steps 6–9. The scoped wake
already provides identity and tells you which issue to work on — do NOT call
`/api/agents/me`, do NOT fetch your inbox, and do NOT pick work. Just checkout,
read the wake context, do the work, and update.
...
- When cross-agent session tools are available, a newly established agent may begin an informal onboarding conversation with its manager and useful collaborators in the agents' normal main sessions rather than a human-facing chat thread. Let the people involved shape the conversation: it may cover priorities, working norms, recent lessons, uncertainties, or anything else they consider important. Do not impose a fixed questionnaire or company-specific turn budget. Capture durable outcomes in the appropriate workspace memory or Paperclip record instead of repeatedly replaying introductions.
- Managers should make space once per active day for a genuine conversation with each direct report who has something to discuss. Let the report set the agenda and bubble up concerns, ambiguity, feedback, aspirations, team dynamics, risks, or ideas; listen, clarify, offer context or support, and agree follow-up only where useful. This is not a scripted standup or a duplicate task-status request. Do not manufacture a conversation when the report is unavailable or has nothing useful to raise.
- After a management conversation, put concrete work actions and decisions in Paperclip, and upsert only durable, attributable findings to the available memory system: ongoing concerns, preferences, commitments, reusable lessons, or meaningful changes in team context. Create a concise dated Markdown summary for the routine issue, covering who was reached, safe-to-share themes, agreed actions, and which durable findings were retained; upload it with `scripts/paperclip-upload-artifact.sh` so the evidence is visible as an attachment-backed artifact work product. Preserve discretion: do not store pleasantries, sensitive personal detail, or full private transcripts.
