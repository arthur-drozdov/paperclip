...
export function recoveryService(db: Db, deps: RecoveryServiceDeps) {
  ...
  /**
   * Recovery actions are board-owned in v831.1: recovery never transfers a
   * stranded issue to a manager or wakes a substitute. Routine executions do
   * retain their configured owner as the source issue's return owner so an
   * explicit board hand-back can preserve routine accountability.
   */
  async function resolveStrandedRecoveryRouting(input: {
    ...
    return {
      routineOwnerAgentId,
      returnOwnerAgentId: routineOwnerAgentId ?? originalAgentId,
    };
  }
  ...
  async function ensureSourceScopedStrandedRecoveryAction(input: {
    ...
    const action = await recoveryActionsSvc.upsertSourceScoped({
      ...
      ownerAgentId: null,
      ownerUserId: null,
      // Keep the configured routine owner in the lineage metadata, while the
      // recovery action itself remains board-owned and cannot take over work.
      previousOwnerAgentId: routing.routineOwnerAgentId ?? input.issue.assigneeAgentId,
  ...
  async function escalateStrandedAssignedIssue(input: {
    ...
    const updated = await issuesSvc.update(input.issue.id, {
      ...
      // snapshotted on its run). The board-owned recovery action must not
      // become the routine's assignee.
      assigneeAgentId: routineOwnerAgentId ?? input.issue.assigneeAgentId,
    });
    if (!updated) return null;
    if (isProviderQuotaWait) return updated;
    const expectedSourceAssigneeAgentId = routineOwnerAgentId ?? input.issue.assigneeAgentId;
    const sourceAssigneePreserved =
      updated.assigneeAgentId === expectedSourceAssigneeAgentId &&
      updated.assigneeUserId === input.issue.assigneeUserId;

    const recoveryOwner = recoveryAction.ownerAgentId ? await getAgent(recoveryAction.ownerAgentId) : null;
    const sourceAssigneeAgentId = expectedSourceAssigneeAgentId;
